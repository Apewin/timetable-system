...
import type { TeachingClass } from "@timetable/core";

function jsonOutput(data: unknown, ok = true, errors: unknown[] = []) {
  console.log(JSON.stringify({ ok, data, errors, warnings: [] }, null, 2));
}

export const teachingClassAdd = defineCommand({
  meta: { name: "add", description: "添加教学班" },
  args: {
    id: { type: "string", description: "教学班ID", required: true },
  ...
  run({ args }) {
    try {
      ...
      const newState = addEntity(state, "teaching_classes", cls);
      writeState(projectPath, newState);

      if (args.json) jsonOutput(cls);
      else {
        console.log(chalk.green("✓ 教学班已添加"));
...
export const teachingClassList = defineCommand({
  meta: { name: "list", description: "列出教学班" },
  args: {
    json: { type: "boolean", description: "JSON格式输出", default: false },
  ...
  run({ args }) {
    try {
      ...
      if (args.json) jsonOutput(list);
        ...
        console.log(`共 ${list.length} 个教学班`);
      }
      process.exit(0);
    } catch (error: any) {
...
export const teachingClassEdit = defineCommand({
  meta: { name: "edit", description: "编辑教学班" },
  args: {
    id: { type: "string", description: "教学班ID", required: true },
  ...
  run({ args }) {
    try {
      ...
      const state = readState(projectPath);
      if (!findEntity(state, "teaching_classes", args.id)) {
        if (args.json) jsonOutput(null, false, [{ code: "NOT_FOUND", msg: `教学班 ${args.id} 不存在` }]);
        else console.error(chalk.red(`错误: 教学班 ${args.id} 不存在`));
        process.exit(3);
      }

      const updates: Partial<TeachingClass> = {};
      ...
      const newState = updateEntity(state, "teaching_classes", args.id, updates);
      writeState(projectPath, newState);

      if (args.json) jsonOutput(findEntity(newState, "teaching_classes", args.id));
      else console.log(chalk.green(`✓ 教学班 ${args.id} 已更新`));
      process.exit(0);
    } catch (error: any) {
...
export const teachingClassRm = defineCommand({
  meta: { name: "rm", description: "删除教学班" },
  args: {
    id: { type: "string", description: "教学班ID", required: true },
    json: { type: "boolean", description: "JSON格式输出", default: false },
  },
  run({ args }) {
    try {
      ...
      const newState = removeEntity(state, "teaching_classes", args.id);
      writeState(projectPath, newState);

      if (args.json) jsonOutput({ removed: args.id });
      else console.log(chalk.green(`✓ 教学班 ${args.id} 已删除`));
      process.exit(0);
    } catch (error: any) {
