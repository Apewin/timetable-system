...
export function workbookSnapshot(workbook, {
  ...
  for (const sheetName of workbook.SheetNames.slice(0, maxSheets)) {
    ...
    const merged_ranges = (sheet['!merges'] || [])
      .filter(range => range.s.r < maxRowsPerSheet && range.s.c < maxColumns)
      .slice(0, 100)
      .map(range => XLSX.utils.encode_range(range));
    sheets.push({ name: sheetName, rows, ...(merged_ranges.length ? { merged_ranges } : {}) });
  }
  return {
    sheets,
...
function schemaFor(expectedType, { courseCatalog = [] } = {}) {
  ...
  if (expectedType === 'elective_selections') {
    return [
      ...
      '第一步必须完成“文档语义判定”（仅在内部推理，不要输出推理过程），再决定是否生成选课：①“课程花名册”——标题、合并标题或工作表名明确写出系统课程目录中的一门或多门课程，姓名在各课程标题下重复出现；应按标题归属导入。②“学生选课汇总表”——每行是一名学生，表头或单元格明确给出 A/B/C 组及课程；应按行导入。③“预分班结果或辅助名单”——只有班级、Block、编号、姓名，或只有姓名汇总，且没有可对应系统课程的明确标题；绝不能当作选课导入。④无法判定——证据不足时不得把名单猜成选课。',
      '在 notes 的第一项写明“识别结论：课程花名册”“识别结论：学生选课汇总表”“识别结论：预分班结果或辅助名单”或“识别结论：无法判定”，并简述证据。若结论为后两类，仍输出标准表头但 rows 必须为空，不能凭学生姓名、常见搭配或课表常识填写 A/B/C。',
      '重点识别规则：一个工作表可能并排放置多份纵向课程名单。例如第一行的不同列分别写“德语名单（8人）”“法语名单（31人）”“日语名单（42人）”。每个标题下同一列的所有非空姓名，都属于该标题对应课程；空列、空行、人数说明和序号都应忽略。',
      '高优先级规则——无表头的三栏横向名单：这类表没有“姓名/课程”表头，第一行的合并标题就是课程归属。例如标题位于 A1:C1“力学基础学生名单”、E1:G1“AP 商业学生名单”、J1:L1“线性代数学生名单”；从第 2 行起，A/E/J 是序号，B/F/K 才是对应的中文姓名。它们是三份独立课程名单，不能当成一张宽表的同一行选课记录，也不能把序号、空白格、标题当作学生。工作簿快照的 merged_ranges 仅辅助判断标题跨度，标题文字与其横向区块才是课程归属依据。',
      '对此结构必须逐个区块向下读取，直到该区块的序号或姓名连续为空；不能因为另一个区块仍有姓名而停止。标题中的“学生名单/名单/AP”等修饰词应剥离后再按课程目录匹配：例如“AP 商业学生名单”对应“商业”。然后合并为一个标准输出 sheet：何子盈应输出为 B组“力学基础”，李之乔应输出为 B组“商业”，厉潇蔓应输出为 B组“线性代数”（实际写入的组别以提供的课程目录为准）。同名学生若出现在不同课程区块，保留其所有已明确出现的组选课；同一组出现两个不同课程才在 notes 中报告冲突。',
      '组别判定是独立步骤：先从标题得到课程名，再只按下方“系统允许课程目录”中该课程所属的 A/B/C 组写入输出列。绝不能使用横向位置、区块数量或出现顺序推断 A/B/C 组别；三个并列区块不等于 A、B、C 三组。本例中力学基础、商业、线性代数均来自 B 组课程目录，所以无论它们位于左、中、右哪个区块，都写入 B组，绝不能写入 C组。',
      '输出前逐项核对每一个非空的 A组/B组/C组单元格：该课程必须出现在系统课程目录的同名组中。若“商业”被写进 C组，或任何课程与目录组别不一致，必须移到目录指定的组；若目录没有该课程则清空该单元格并在 notes 说明，不能保留错误组别。',
      '高优先级规则——高三 A 组英美课程横向花名册：同一行也可能有三份 A 组课程名单，标题例如 A1:B1“英美文学史及选读（20人）-416教室”、D1:E1“AP LC(33人)-310教室”、G1:H1“AP Lit.&Com.(27人)-411教室”。括号内人数和“-教室”只是元数据，必须剥离；左列 A/D/G 是序号，右列 B/E/H 是姓名。课程映射固定为：英美文学史及选读 → Honor 英美文学史及选读；AP LC 或 LC → AP Language and Composition；AP Lit.&Com. 或 AP Lit Com → AP Literature and Composition。三个区块均写入 A组，绝不能因横向顺序把后两栏写入 B组或 C组。',
      '这三份名单是同一“高三 A 组选课”中的不同课程，不是 AP 分班、教室安排或三种 A/B/C 组别。逐个区块向下读到该区块名单结束；输出时一人一行、课程写入 A组。若同一学生同时出现在这三份名单中，说明其 A 组课程冲突，必须在 notes 说明，不能把其中一门挪入 B/C 组。',
      '课程名单也可能是其他“并排的多列区块”，而不是单列；标题下每行可能包含序号、班级/年份数字和姓名。只提取确实像姓名的文本，数字如“1”“25”“26”不是学生姓名。姓名多数位于序号右侧，也可能因合并单元格或格式错位落在同一标题区块内的其他文本列，仍应归入最近且同一横向区块的课程标题。',
      '同一文件还可能包含一个只有姓名、没有课程标题的汇总工作表（例如 Sheet2）。该汇总表不能用于推断课程归属，必须完全忽略并在 notes 说明；不得把其中学生补入任意课程。',
